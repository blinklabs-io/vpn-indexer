...
func (a *Api) verifyCOSESignature(
	...
	vkeyHash := lcommon.Blake2b224Hash([]byte(vkey.(ed25519.PublicKey)))
...
func (a *Api) wgRegisterImpl(
	...
	// If this fails, nothing is persisted and we can safely return an error
	if s3Client != nil {
		if err := s3Client.SavePeerToS3(
			req.innerClientID,
			req.WGPubkey,
			assignedIP,
		); err != nil {
	...
	if err := a.db.AddWGPeer(
		...
		slog.Error("failed to add WG peer to database", "error", err)
		// Don't roll back S3 - it's the source of truth
		// The DB will be rebuilt from S3 on next startup
		writeErrorResponse(
			w,
			http.StatusInternalServerError,
			"Internal server error",
...
func (a *Api) wgProfileImpl(
	...
	if peer == nil || peer.AssetName == nil ||
...
func (a *Api) wgPeerDeleteImpl(
	...
	if s3Client != nil {
		if err := s3Client.RemovePeerFromS3(
			req.innerClientID,
			req.WGPubkey,
		); err != nil {
	...
	if err := a.db.DeleteWGPeer(req.WGPubkey); err != nil {
		slog.Error("failed to delete WG peer from database", "error", err)
		writeErrorResponse(
			w,
			http.StatusInternalServerError,
			"Internal server error",
